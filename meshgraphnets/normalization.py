...
class Normalizer(snt.AbstractModule):
  ...
  def _std_with_epsilon(self):
    ...
    # return tf.cond(self._num_accumulations<=0., lambda:1., lambda: tf.math.maximum(std, self._std_epsilon))
    # return tf.math.maximum(std, self._std_epsilon)
    f_empty = safe_count-self._acc_count
    return tf.math.maximum(std, f_empty+(1-f_empty)*self._std_epsilon)
